...
namespace CalamityClickers.Content.NPCs
{
    public class LecherousOrbClicker : ModNPC
    {
        ...
        public override void ApplyDifficultyAndPlayerScaling(int numPlayers, float balance, float bossAdjustment) => NPC.lifeMax = 180000; public override void AI()
        {
            ...
            if (Main.myPlayer == NPC.target)
            {
                ...
                if (!Owner.CalClicker().enchLecherous)
                {
                    NPC.active = false;
                    NPC.life = 0;
                    NPC.HitEffect();
                ...
                Vector2 destination = Main.MouseWorld + Vector2.UnitX.RotatedBy(Main.GlobalTimeWrappedHourly) * 10;
                NPC.Center = Vector2.Lerp(NPC.Center, destination, 0.035f).MoveTowards(destination, 8f);
                if (NPC.WithinRange(destination, 5f))
                    NPC.Center = destination;
